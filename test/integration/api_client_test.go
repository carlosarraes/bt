...
func (suite *APIClientIntegrationSuite) setupAuthentication(cfg *config.Config) auth.AuthManager {
	...
	if token != "" {
		...
		authManager, err = auth.NewAccessTokenAuth(token)
		require.NoError(suite.T(), err)
		...
		suite.T().Skip("No authentication credentials provided. Set BITBUCKET_TOKEN or BITBUCKET_USERNAME/BITBUCKET_PASSWORD")
	}
...
func BenchmarkRealAPICall(b *testing.B) {
	...
	token := os.Getenv("BITBUCKET_TOKEN")
	if token == "" {
		b.Skip("No BITBUCKET_TOKEN provided for benchmark")
	}
